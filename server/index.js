...
const API_KEY = process.env.VITE_API_KEY || process.env.GOOGLE_API_KEY;
...
const getMockData = (query, isFallback = false) => {
...
// Using process.env.VITE_API_KEY directly in initialization
...
app.post('/api/generate-icp', async (req, res) => {
    ...
    if (!API_KEY) {
        ...
        return res.json(getMockData(query, false));
    }
    ...
    `;
        ...
        const validUris = new Set(sources.map((s) => s.uri));
        ...
        res.json(getMockData(query, true));
    }
});

app.post('/api/enrich-role', async (req, res) => {
    const { companyName, roleTitle } = req.body;

    if (!API_KEY || !ai) {
        console.warn("No API Key or AI client. Skipping enrichment.");
        return res.json({ name: "John Doe (Demo)", linkedin: "#", context: "Demo Mode - No API Key" });
    }
